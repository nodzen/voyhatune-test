...
grep -q 'pushFrozenApps(context, prefs)' "$SYNC" || fail "startup/wake sync omits frozen launcher apps"
grep -q 'FROZEN_APPS_CONFIG' "$SYNC" || fail "frozen launcher app config action missing"
grep -q 'frozenAppsEnabled' "$ROOT/RestoreMode/app/src/main/java/ru/big/town/restoremode/FrozenAppStore.java" \
    || fail "frozen apps master switch is missing"
grep -q 'isEnabled(prefs)' "$ROOT/RestoreMode/app/src/main/java/ru/big/town/restoremode/FrozenAppStore.java" \
    || fail "frozen apps switch does not publish restore snapshot when disabled"
grep -q 'switchFrozenApps' "$ROOT/RestoreMode/app/src/main/res/layout/activity_advance.xml" \
    || fail "frozen apps master switch is missing from the UI"
grep -q 'mirrorFrozenApps(context, intent)' "$NATIVE_CONFIG" || fail "Native does not receive frozen launcher app config"
grep -q 'voyahtune_frozen_apps' "$NATIVE_BRIDGE" || fail "frozen launcher package list is not mirrored"
grep -q 'onRestoreFrozenApps' "$ADVANCE" || fail "frozen app blacklist has no restore-all action"
grep -q 'buttonRestoreFrozenApps' "$ROOT/RestoreMode/app/src/main/res/layout/activity_advance.xml" \
    || fail "frozen app blacklist has no restore-all button"
grep -q 'dock1Long' "$SYNC" || fail "dock long actions are not published"
grep -q 'dock2Long' "$SYNC" || fail "second dock long actions are not published"
grep -q 'OPEN_DOCK_ACTION' "$NATIVE_BRIDGE" || fail "Native dock action bridge is missing"
grep -q 'OPEN_DOCK_ACTION' "$ROOT/Packaging/inject/launcherdock.js" \
    || fail "launcher long press does not route dock actions"
