...
Права каталогов Frida-инфраструктуры ставятся и проверяются перед инъекцией — установщиком и телом
`voyahtune.load.sh`, — потому что агент работает под UID целевого процесса, а не под root:
`/data/local` = `0751 root:root`, `/data/local/bin` = `0755 root:root`,
`/data/local/tmp` = `0771 shell:shell` (2000:2000). Режим задаётся пятью октальными цифрами, чтобы
явно снять унаследованные setgid/sticky биты. `chmod` никогда
не рекурсивный: конфиги агентов остаются `0644`, а root-состояние воркеров в `/data/local/tmp` —
приватным. Без этого каталог с режимом `2700`, унаследованный от прошивки или стороннего инструмента,
молча ломает клавиатурные агенты: они читают свой JSON через `java.io.FileInputStream` уже под UID
qgime, и файла `0644` мало, если родительский каталог непроходим для чужого UID.

Сверка результата — **только диагностическая**, и это принципиально: `chown`/`chmod` в цепочке `&&`
и так возвращают ненулевой код при реальном сбое, а вот `stat -c %a` рендерит режим как `751` или
`0751` в зависимости от сборки toybox. Строгое сравнение строк из-за одной разницы в
форматировании не должно ни убивать загрузчик (тогда не поднимается весь Frida-слой), ни блокировать
установку. Поэтому `voyahtune.load.sh` вызывает `check_data_dir` и только пишет в лог
`unexpected mode`, а `test_hook_manifest_status`-подобного fail-closed здесь нет и быть не должно.
`test_loader_fault_backoff.sh` это фиксирует: внутри фатального блока не должно быть ни одного
`stat`.

Записи лоадера с разделителем `|` читаются только в экранированном виде (`${v%%\|*}`, `${v#*\|}`).
Штатный `/system/bin/sh` — это MIRBSD KSH R57, где неэкранированный `|` внутри шаблона `${...}`
трактуется как **альтернация**, а не как литерал: `%%|*` схлопывает всю строку, а `#*|` не отрезает
ничего, и любая корректная запись начинает отвергаться собственной проверкой. bash и dash, на которых
исполняются гварды, трактуют `|` литерально, поэтому в `test_loader_fault_backoff.sh` добавлены
статическая проверка и поведенческий прогон под ksh-совместимым шеллом.

Инъекция в keymanager считается успешной **только** по маркеру готовности, который агент сам печатает
в stdout: `Log.i()` уходит исключительно в logcat и на stdout `frida-inject` не попадает, поэтому
проверять его grep-ом бессмысленно, а код возврата инжектора доказывает лишь то, что `frida`
запустился, но не что хуки встали. Поэтому `steeringwheelkeys.js` дублирует строку готовности в
`console.log`, а `load.bin` больше не принимает `[ "$KM_RC" -eq 0 ]` как подтверждение. Воркер
перепроверяет exact identity уже после `fork`: за время запуска keymanager мог перезапуститься, и
инъектить в уехавший процесс бессмысленно — маркер достался бы чужому адресату.
...
каждый Frida-агент через временный файл с атомарным `mv`. Целостность агентов при этом **проверяется
трижды** (утратившая силу запись об отсутствии hook-manifest в этом файле была неверной):

- `install.sh` сверяет `schemaVersion`, ровно восемь записей (`-ne 8`) и каждую из восьми
  `verify_hook_manifest_entry` — id, процесс, скрипт и SHA-256;
- `install.bat` делает то же через PowerShell с восемью ожидаемыми записями и восемью
  `call :compute_sha256`;
- `load.bin` сверяет хэш каждого агента непосредственно перед инъекцией
  (`manifest_script_integrity`) и при расхождении закрывает набор fail-closed, а не допускает
  подменённого агента в OEM-процесс.

Восемь записей — это все агенты, кроме `app_client.js`: он инжектится по PID стороннего приложения и
в манифест не входит. Счётчик восьми зафиксирован также в `test_hook_manifest_status.sh` (строки 26
и 93). Трекерный манифест перегенерируется только `./Packaging/update_hook_manifest.sh`; сборка
релиза (`make_release.sh`) его не проверяет, а генерирует свой для staging-копии, поэтому устаревший
манифест в git сам по себе release не ломает — его ловит `test_hook_manifest_status.sh`. После запуска
`load.bin` сверяет целостность набора и переходит к обнаружению целевых процессов и установке агентов.
