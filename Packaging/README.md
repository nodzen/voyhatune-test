...
В full-разделе настроек есть blacklist заморозки приложений. Пользователь вручную выбирает
пакеты кнопкой «Заморозить приложения» и отдельно включает тумблер; автоматически ничего не
замораживается. Выбранный пакет Native сначала
force-stop'ит, затем переводит через PackageManager в `DISABLED_USER`: он исчезает из All Apps,
его сервисы/ресиверы не запускаются, но APK и данные не удаляются. Снятие отметки возвращает
исходное состояние enabled/disabled, сохранённое перед заморозкой. Радио, Bluetooth-телефон,
launcher, CAN, климат, vehicle/cluster и другие критические контуры не предлагаются к заморозке;
из OEM system-uid разрешены только проверенные необязательные UI/media-пакеты.
...
Неизвестный чужой overlay не перезаписывается и не удаляется. Основной Windows-установщик в конце
спрашивает, включать ли Yandex DNS; при отказе текущее состояние остаётся без изменений. Если нужен
только DNS без переустановки приложений, отдельно запускается `install-yandex-dns.sh` на macOS/Linux
или `install-yandex-dns.bat` на Windows. Оба используют тот же вопрос и тот же helper.
...
«Применить» применяет его сразу, а автоматическая restore-цепочка стартует сразу после события
старта/пробуждения и ждёт только фактической готовности провайдера и CAN.
...
восстановление запускается сразу после wake-события и коротко повторяет попытку только если Android
11/OEM CanBus ещё не готовы; ручное применение остаётся немедленным.
