...
if errorlevel 1 exit /b 1
adb.exe shell "for removed_path in /system/etc/init/voyahtune.load.rc /system/etc/init.voyahtune.load.sh /system/etc/init/voyahtune.load.sh /data/local/bin/vd_bypass.js /data/local/bin/steeringwheelkeys.js /data/local/bin/launcherdock.js /data/local/bin/multidisplay.js /data/local/bin/app_client.js /data/local/bin/app_client.js.voyahtune.new /data/local/bin/fullscreen_client.js /data/local/bin/fullscreen_client.js.voyahtune.new /data/local/bin/apollo_tech.js /data/local/bin/keyboard_lock_en.js /data/local/bin/keyboard_ru.js /data/local/bin/voyahtune-hook-manifest.json /data/local/tmp/voyahtune-hook-status.v1; do [ ! -e $removed_path ] && [ ! -L $removed_path ] || exit 1; done; sync"
adb.exe shell "for removed_path in /data/local/bin/instrumentcard.js /data/local/bin/instrumentcard.js.voyahtune.new /data/local/tmp/voyahtune_instrumentcard.pid /data/local/tmp/voyahtune_instrumentcard.attempt /data/local/tmp/voyahtune_instrumentcard.txt /data/local/tmp/voyahtune_instrumentcard.txt.try; do [ ! -e $removed_path ] && [ ! -L $removed_path ] || exit 1; done"
if errorlevel 1 exit /b 1
exit /b %ERRORLEVEL%
