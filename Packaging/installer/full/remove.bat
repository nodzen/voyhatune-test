...
adb.exe shell "ps -ef | grep frida-inject | grep -E 'vd_bypass|steeringwheelkeys|launcherdock|multidisplay|apollo_tech|keyboard_lock_en|keyboard_ru|instrumentcard|app_client|fullscreen_client' | grep -v grep | awk '{print $2}' | xargs kill -9"
...
adb.exe shell "rm -f /data/local/bin/instrumentcard.js /data/local/bin/instrumentcard.js.voyahtune.new /data/local/tmp/voyahtune_instrumentcard.pid /data/local/tmp/voyahtune_instrumentcard.attempt /data/local/tmp/voyahtune_instrumentcard.txt /data/local/tmp/voyahtune_instrumentcard.txt.try"
...
adb.exe shell "rm -f /data/local/bin/instrumentcard.js /data/local/bin/instrumentcard.js.voyahtune.new /data/local/tmp/voyahtune_instrumentcard.pid /data/local/tmp/voyahtune_instrumentcard.attempt /data/local/tmp/voyahtune_instrumentcard.txt /data/local/tmp/voyahtune_instrumentcard.txt.try && for path in /data/local/bin/instrumentcard.js /data/local/bin/instrumentcard.js.voyahtune.new /data/local/tmp/voyahtune_instrumentcard.pid /data/local/tmp/voyahtune_instrumentcard.attempt /data/local/tmp/voyahtune_instrumentcard.txt /data/local/tmp/voyahtune_instrumentcard.txt.try; do if [ -e \"$path\" ] || [ -L \"$path\" ]; then exit 1; fi; done"
